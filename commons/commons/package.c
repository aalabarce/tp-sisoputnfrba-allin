...
t_stream * paquetizar(int tipoEstructura, void * estructuraOrigen){
	...
	switch (tipoEstructura){
			...
			case D_STRUCT_ACTUALIZARALGORITMO:
				paquete = paquetizarStruct_algoritmo((t_struct_algoritmo *) estructuraOrigen);
				break;
			case D_STRUCT_NOMBRENIVEL:
				paquete = paquetizarStruct_nombreNivel((t_struct_nombreNivel *) estructuraOrigen);
				break;
			case D_STRUCT_CONECTAME:
				paquete = paquetizarStruct_conectame((t_struct_nombreNivelYSimbolo *) estructuraOrigen);
				break;
			...
			case D_STRUCT_DATOSNIVEL:
				...
				break;
			case D_STRUCT_DATONIVELYRECURSO:
				paquete = paquetizarStruct_datosNivel2((t_struct_datosNivel *) estructuraOrigen);
				break;
			...
			case D_STRUCT_RECURSOSLIBERADOS:
				...
				break;
			case D_STRUCT_POSXPOSY_SIMBOLO:
				paquete = paquetizarStruct_posicionYSimbolo((t_struct_posxposy_simbolo *) estructuraOrigen);
				break;
			case D_STRUCT_SIMBOLO_DIBUJAME:
				paquete = paquetizarStruct_simboloDibujame((t_struct_simbolo_dibujame *) estructuraOrigen);
				break;
			case D_STRUCT_PEDIRPOS_RECURSO:
				paquete = paquetizarStruct_pedirPosicionRecurso((t_struct_pedir *) estructuraOrigen);
				break;
			case D_STRUCT_ASIGNAR_RECURSO:
				paquete = paquetizarStruct_asignarRecurso((t_struct_asignar *) estructuraOrigen);
				break;
		}
...
t_stream * paquetizarStruct_charPedirPosicionRecurso(t_struct_pedirPosicionRecurso * estructuraOrigen){
	...
	paquete->length = sizeof(t_header) + sizeof(char);
...
/*
 * Nombre: paquetizarStruct_algoritmo/1
 * Argumentos:
 * 		- estructuraOrigen
 *
 * Devuelve:
 ...
 * Funcion: crearDataConHeader(3, length) -> reserva la memoria para el data del paquete, y le agrega el header.
 */
t_stream * paquetizarStruct_algoritmo(t_struct_algoritmo * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + strlen(estructuraOrigen->algoritmo) + 1 + sizeof(estructuraOrigen->quantum) + sizeof(estructuraOrigen->retardo);

	char * data = crearDataConHeader(D_STRUCT_ACTUALIZARALGORITMO, paquete->length); //creo el data

	int tamanoTotal = sizeof(t_header), tamanoDato = 0;

	memcpy(data + tamanoTotal, estructuraOrigen->algoritmo, tamanoDato = strlen(estructuraOrigen->algoritmo)+1);		//copio a data el algortimo (char *).

	tamanoTotal += tamanoDato;

	memcpy(data + tamanoTotal, &estructuraOrigen->quantum, tamanoDato = sizeof(estructuraOrigen->quantum));		//copio a data el quantum (unsigned int).

	tamanoTotal += tamanoDato;

	memcpy(data + tamanoTotal, &estructuraOrigen->retardo, tamanoDato = sizeof(estructuraOrigen->retardo));		//copio a data el retardo (unsigned int).
...
/*
 * Nombre: paquetizarStruct_conectame/1
 * Argumentos:
 * 		- estructuraOrigen
 *
 * Devuelve:
 ...
 * Funcion: crearDataConHeader(3, length) -> reserva la memoria para el data del paquete, y le agrega el header.
 */
t_stream * paquetizarStruct_conectame(t_struct_nombreNivelYSimbolo * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + strlen(estructuraOrigen->nombre) + 1 + sizeof(char);

	char * data = crearDataConHeader(D_STRUCT_CONECTAME, paquete->length); //creo el data

	int tamanoTotal = sizeof(t_header), tamanoDato = 0;

	memcpy(data + tamanoTotal, estructuraOrigen->nombre,tamanoDato = strlen(estructuraOrigen->nombre)+1);		//copio a data el string del nombre.

	tamanoTotal += tamanoDato;

	memcpy(data + tamanoTotal,&estructuraOrigen->simboloPersonaje,tamanoDato = sizeof(char));
...
t_stream * paquetizarStruct_asignarRecurso(t_struct_asignar * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + sizeof(t_struct_asignar);

	char * data = crearDataConHeader(D_STRUCT_ASIGNAR_RECURSO, paquete->length); //creo el data

	memcpy(data + sizeof(t_header), estructuraOrigen, sizeof(t_struct_asignar));	//copio a data la estructura.
...
t_stream * paquetizarStruct_pedirPosicionRecurso(t_struct_pedir * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + sizeof(t_struct_pedir);

	char * data = crearDataConHeader(D_STRUCT_PEDIRPOS_RECURSO, paquete->length); //creo el data

	memcpy(data + sizeof(t_header), estructuraOrigen, sizeof(t_struct_pedir));	//copio a data la estructura.
...
t_stream * paquetizarStruct_datosNivel(t_struct_datosNivel * estructuraOrigen){
	...
	paquete->length = sizeof(t_header) + strlen(estructuraOrigen->nombre) + 1 + strlen(estructuraOrigen->recursos) + 1 + strlen(estructuraOrigen->algoritmo) + 1 + sizeof(estructuraOrigen->quantum) + sizeof(estructuraOrigen->retardo);
...
t_stream * paquetizarStruct_datosNivel2(t_struct_datosNivel * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + strlen(estructuraOrigen->nombre) + 1 + strlen(estructuraOrigen->recursos) + 1 ;

	char * data = crearDataConHeader(D_STRUCT_DATONIVELYRECURSO, paquete->length); //creo el data
...
/*
 * Nombre: paquetizarStruct_posicionYSimbolo /1
 * Argumentos:
 * 		- estructuraOrigen
 *
 * Devuelve:
 ...
 * Funcion: crearDataConHeader(4, length) -> reserva la memoria para el data del paquete, y le agrega el header.
 */
t_stream * paquetizarStruct_posicionYSimbolo(t_struct_posxposy_simbolo * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + sizeof(t_struct_posxposy_simbolo);

	char * data = crearDataConHeader(D_STRUCT_POSXPOSY_SIMBOLO, paquete->length); //creo el data

	memcpy(data + sizeof(t_header), estructuraOrigen, sizeof(t_struct_posxposy_simbolo));	//copio a data la estructura.
...
/*
 * Nombre: paquetizarStruct_simboloDibujame/1
 * Argumentos:
 * 		- estructuraOrigen
 *
 * Devuelve:
 ...
 * Funcion: crearDataConHeader(2, length) -> reserva la memoria para el data del paquete, y le agrega el header.
 */
t_stream * paquetizarStruct_simboloDibujame(t_struct_simbolo_dibujame * estructuraOrigen){

	t_stream * paquete = malloc(sizeof(t_stream));		//creo el paquete

	paquete->length = sizeof(t_header) + sizeof(unsigned int);

	char * data = crearDataConHeader(D_STRUCT_SIMBOLO_DIBUJAME, paquete->length); //creo el data
...
void * despaquetizar(uint8_t tipoEstructura, char * dataPaquete, uint16_t length){
	...
	switch (tipoEstructura){
			...
			case D_STRUCT_NOMBRENIVEL:
				...
				break;
			case D_STRUCT_CONECTAME:
				estructuraDestino = despaquetizarStruct_conectame(dataPaquete, length);
				break;
			case D_STRUCT_PERSONAJESINTERBLOQUEADOS:
			...
			case D_STRUCT_DATOSNIVEL:
				...
				break;
			case D_STRUCT_DATONIVELYRECURSO:
				estructuraDestino = despaquetizarStruct_datosNivel2(dataPaquete, length);
				break;
			case D_STRUCT_RECURSOSASIGNADOSYSOBRANTES:
				estructuraDestino = despaquetizarStruct_recursosAsignadosYSobrantes(dataPaquete, length);
				break;
			case D_STRUCT_ACTUALIZARALGORITMO:
				estructuraDestino = despaquetizarStruct_algoritmo(dataPaquete, length);
				break;
			case D_STRUCT_POSXPOSY_SIMBOLO:
				estructuraDestino = despaquetizarStruct_posxposy_simbolo(dataPaquete, length);
				break;
			case D_STRUCT_ASIGNAR_RECURSO:
				estructuraDestino = despaquetizarStruct_asignarRecurso(dataPaquete, length);
				break;
			case D_STRUCT_PEDIRPOS_RECURSO:
				estructuraDestino = despaquetizarStruct_pedirPosicionRecurso(dataPaquete, length);
				break;
		}
...
/*
 * Nombre: despaquetizarStruct_algoritmo/2
 * Argumentos:
 * 		- char * dataPaquete
 * 		- length
 *
 * Devuelve:
 * 		una estructura de tipo D_STRUCT_ACTUALIZARALGORITMO.
 *
 */
t_struct_algoritmo * despaquetizarStruct_algoritmo(char * dataPaquete, uint16_t length){

	t_struct_algoritmo * estructuraDestino = malloc(sizeof(t_struct_algoritmo));

	int tamanoTotal = 0, tamanoDato = 0;

	tamanoTotal = tamanoDato;

	for(tamanoDato = 1; (dataPaquete + tamanoTotal)[tamanoDato -1] != '\0';tamanoDato++); //incremento tamanoDato, hasta el tamaño de la cadena con el algoritmo.

	estructuraDestino->algoritmo = malloc(tamanoDato);
	memcpy(estructuraDestino->algoritmo, dataPaquete + tamanoTotal, tamanoDato); //copio el algoritmo a la estructura

	tamanoTotal += tamanoDato;

	memcpy(&estructuraDestino->quantum, dataPaquete + tamanoTotal, sizeof(int)); //copio el quantum a la estructura

	tamanoTotal += sizeof(int);

	memcpy(&estructuraDestino->retardo, dataPaquete + tamanoTotal, sizeof(int)); //copio el retardo a la estructura
...
t_struct_datosNivel * despaquetizarStruct_datosNivel(char * dataPaquete, uint16_t length){
	...
	memcpy(&estructuraDestino->quantum, dataPaquete + tamanoTotal, sizeof(unsigned int)); //copio el quantum a la estructura

	tamanoTotal += sizeof(unsigned int);

	memcpy(&estructuraDestino->retardo, dataPaquete + tamanoTotal, sizeof(unsigned int)); //copio el retardo a la estructura
 ...
 *
 */
t_struct_datosNivel * despaquetizarStruct_datosNivel2(char * dataPaquete, uint16_t length){
	t_struct_datosNivel * estructuraDestino = malloc(sizeof(t_struct_datosNivel));
 ...
 *
 */
t_struct_nombreNivelYSimbolo * despaquetizarStruct_conectame(char * dataPaquete, uint16_t length){
	t_struct_nombreNivelYSimbolo * estructuraDestino = malloc(sizeof(t_struct_nombreNivelYSimbolo));
	...
	memcpy(&estructuraDestino->simboloPersonaje, dataPaquete + tamanoTotal, sizeof(char)); //copio el string a la estructura
...
t_struct_posxposy_simbolo * despaquetizarStruct_posxposy_simbolo(char * dataPaquete, uint16_t length){
	t_struct_posxposy_simbolo * estructuraDestino = malloc(sizeof(t_struct_posxposy_simbolo));

	memcpy(estructuraDestino, dataPaquete, sizeof(t_struct_posxposy_simbolo)); //copio el data del paquete a la estructura

	return estructuraDestino;
}


t_struct_pedir * despaquetizarStruct_pedirPosicionRecurso(char * dataPaquete, uint16_t length){
	t_struct_pedir * estructuraDestino = malloc(sizeof(t_struct_pedir));

	memcpy(estructuraDestino, dataPaquete, sizeof(t_struct_pedir)); //copio el data del paquete a la estructura

	return estructuraDestino;
}

t_struct_asignar * despaquetizarStruct_asignarRecurso(char * dataPaquete, uint16_t length){
	t_struct_asignar * estructuraDestino = malloc(sizeof(t_struct_asignar));

	memcpy(estructuraDestino, dataPaquete, sizeof(t_struct_asignar)); //copio el data del paquete a la estructura
