...
int epoll_eliminarSocketCliente(int epollfd, int socketCliente) {
	int ret;
	ret = epoll_ctl(epollfd, EPOLL_CTL_DEL, socketCliente, 0);
	if (ret == -1) {
		perror("epoll_ctl: socket cliente");	// CAMBIAR POR UN LOG
		return -1;
	}
 ...
 * uniqueHandler trata si uniqueSocket tuvo actividad. Generalmente usado para servidores. Si no se quiere utilizar, uniqueSocket poner -1 y uniqueHandler NULL.
...
//int socket_enviarSignalaNivel(int socketReceptor, t_signal signal){
//	t_struct_signal *structSignal = malloc(sizeof(t_struct_signal));
//
//	structSignal->signal = signal;
//
