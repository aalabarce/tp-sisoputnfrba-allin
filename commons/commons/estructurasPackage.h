...
#define Termine_Juego 21
#define No_Termine_Juego 22
#define S_Enemigo_Mato_Personaje 23
#define S_Termine_Por_Control_C 24
...
#define D_STRUCT_SIGNAL 20
#define D_STRUCT_DATONIVELYRECURSO 5
...
#define D_STRUCT_RECURSOSNIVEL 16
#define D_STRUCT_CONECTAME 17
#define D_STRUCT_ACTUALIZARALGORITMO 18
#define D_STRUCT_POSXPOSY_SIMBOLO 19
#define D_STRUCT_SIMBOLO_DIBUJAME 21
#define D_STRUCT_PEDIRPOS_RECURSO 22
#define D_STRUCT_ASIGNAR_RECURSO 23
...
//Estructura tipo STRUCT_PEDIRRECURSO (7)
typedef struct struct_pedirRecursoConSimbolo {
	char letra;
	char simbolo;
} __attribute__ ((__packed__)) t_struct_pedirRecursoConSimbolo;
 ...
 */
typedef struct struct_datosNivel {
	char * nombre;
	char * recursos;
	char * algoritmo;
...
typedef struct struct_nombreNivelYSimbolo {
	char * nombre;
	char simboloPersonaje;
} __attribute__ ((__packed__)) t_struct_nombreNivelYSimbolo;
...
/*Estructura tipo D_STRUCT_ACTUALIZARALGORITMO(18)
 * La usa el nivel para mandarle al orquestador:
 * el nuevo algoritmo cuando se actualiza por inotify
*/
typedef struct struct_algoritmo{
	char * algoritmo;
	int quantum;
	int retardo;
} __attribute__ ((__packed__)) t_struct_algoritmo;

/*Estructura tipo STRUCT_POSXPOSY_SIMBOLO(19)
 * Va a enviar la posicion en x y en y (dos unsigned int) en la que se encuentra el personaje o el recurso; Y el simbolo en char.
 */
typedef struct struct_posxposy_simbolo {
	char simbolo;
	unsigned int posX;
	unsigned int posY;
} __attribute__ ((__packed__)) t_struct_posxposy_simbolo;


//Estructura tipo STRUCT_SIMBOLO_DIBUJAME(21)
typedef struct struct_simbolo_dibujame {
	char letra;
} __attribute__ ((__packed__)) t_struct_simbolo_dibujame;

//Estructura tipo STRUCT_PEDIRPOS_RECURSO(22)
typedef struct struct_pedir {
	char recurso;
	char personaje;
} __attribute__ ((__packed__)) t_struct_pedir;

//Estructura tipo STRUCT_ASIGNAR_RECURSO(23)
typedef struct struct_asignar {
	char recurso;
	char personaje;
} __attribute__ ((__packed__)) t_struct_asignar;
